...
import { fmtUsd, fmtShort, fmtK, fmtNum, short, fmtTime } from "../api.js";
...
export default async function livePage(root) {
  ...
  const render = () => {
    ...
    $("liq").innerHTML = s.liquidations.length ? s.liquidations.slice(0, 150).map((d, i) =>
      `<div class="${i === 0 ? "new" : ""}"><span class="t">${fmtTime(d.ts)}</span>${sideTag(d.side === "Buy")}<span class="sym">${d.symbol}</span><span>${fmtK(d.amount)} @ ${d.price}</span><b>${fmtShort(d.usd)}</b><span class="dim">${+d.remainAmount > 0 ? "partial" : "full"}</span><span class="addr">${short(d.walletId)}${d.n > 1 ? ` ×${d.n}` : ""}</span></div>`).join("")
    ...
    $("whales").innerHTML = s.whales.length ? s.whales.slice(0, 150).map((d, i) =>
      `<div class="${i === 0 ? "new" : ""}"><span class="t">${fmtTime(d.createdAt)}</span><span class="side ${d.side === "Buy" ? "up" : "down"}">${d.side}</span><span class="sym">${d.s}</span><span>${fmtK(d.size)} @ ${d.price}</span><b>${fmtShort(d.usd)}</b><span class="dim">blk ${d.block}</span></div>`).join("")
