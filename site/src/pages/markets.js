...
export default async function markets(root) {
  ...
  $("tiles").innerHTML = tile("Markets", syms.length, `${syms.filter((s) => s.assetClass === "Rwa").length} RWA · ${syms.length - syms.filter((s) => s.assetClass === "Rwa").length} crypto`) +
    tile("24h volume", fmtShort(tot24), fmtUsd(tot24)) + tile("RWA share", fmtPct(rwa24 / tot24, 1), "of 24h volume") + tile("Open interest", fmtShort(oiUsd), `mark value · ${fmtUsd(oiUsd)}`);
  ...
  chart($("vol"), {
    xAxis: { type: "category", data: labels }, yAxis: { type: "value", axisLabel: { formatter: (v) => fmtShort(v) } }, legend: { top: 0 }, tooltip: { trigger: "axis", valueFormatter: (v) => fmtShort(v) },
